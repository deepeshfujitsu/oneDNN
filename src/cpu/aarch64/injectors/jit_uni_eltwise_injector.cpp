...
template <cpu_isa_t isa>
void jit_uni_eltwise_injector_f32<isa>::tanh_polynomial_approx_compute_vector_fwd(const TRegS &vmm_src) {
    ...
    auto gather_coefficient = [&](TRegS vmm_coeff, int coeff_idx, TRegS vmm_pol_idx) {                     
             h->add_imm(h->X_TMP_1, x_table, table_off(tanh_pol_table, coeff_idx * tanh_n_polynomials), h->X_TMP_0);
             h->ld1w(ZRegS(IDX(vmm_coeff)), p_all, ptr(h->X_TMP_1, ZRegS(IDX(vmm_pol_idx)), SXTW));                
    };
    ...
    h->sub(ZRegS(IDX(vmm_indices)), ZRegS(IDX(vmm_src_pos)), ZRegS(IDX(table_val(tanh_idx_bias,z_tmp))));
    h->and_(ZRegD(IDX(vmm_indices)), ZRegD(IDX(vmm_indices)), ZRegD(IDX(table_val(tanh_idx_mask,z_tmp))));
    h->lsr(ZRegD(IDX(vmm_indices)), ZRegD(IDX(vmm_indices)), 20);

    // Argument reduction
     h->and_(ZRegD(IDX(vmm_src_shift)), ZRegD(IDX(vmm_src_pos)), ZRegD(IDX(table_val(tanh_idx_mask,z_tmp))));
    ...
    h->and_(ZRegD(IDX(vmm_sign)), ZRegD(IDX(vmm_src)), ZRegD(IDX(table_val(sign_mask,z_tmp))));
