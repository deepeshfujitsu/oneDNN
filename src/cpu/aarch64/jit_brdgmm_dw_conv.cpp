...
//     const auto isa = sve_512;
    ...
    if (cd.dilates[0] != 0 || cd.dilates[1] != 0 || (is_3d && cd.dilates[2] != 0))
    ...
    const bool any_eligible
            = (cd.prop_kind == prop_kind::forward_inference || is_int8 || is_3d);
...
status_t brdgmm_dw_convolution_fwd_t<isa>::pd_t::init_brdgmm_conf() {
    ...
    if ((jcp.mb * jcp.od * jcp.oh) % jcp.nthr != 0) {
        ...
        {
            const size_t work_amount = jcp.mb * jcp.nb_ch * jcp.oh * jcp.od * jcp.nb_ow;
            if (work_amount % jcp.nthr == 0) {
                const size_t work_per_thr = div_up(work_amount, jcp.nthr);
                ...
                const int max_ch_block2 = is_superset(jcp.isa, sve_512)
                        ? 4
                        : bcp_0.ld_block;
                jcp.nb_ch_blocking
...
template <cpu_isa_t isa>
status_t brdgmm_dw_convolution_fwd_t<isa>::execute(const exec_ctx_t &ctx) const {
    ...
    parallel(jcp.nthr, [&](const int ithr, const int nthr) {
        ...
        while (iwork < end) {
            ...
            assert(static_cast<int>(pd()->batches_.size()) >= (bi + 1) * max_bs);
